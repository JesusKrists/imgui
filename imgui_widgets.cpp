...
bool ImGui::TreeNodeEx_IconText(ImTextureID textureID, const char* label, ImGuiTreeNodeFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;

    return TreeNodeBehavior_IconText(window->GetID(label), flags, textureID, label, NULL);
}

bool ImGui::TreeNodeEx_IconText(ImTextureID closeTextureID, ImTextureID openTextureID, const char* label, ImGuiTreeNodeFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;

    return TreeNodeBehavior_IconText(window->GetID(label), flags, closeTextureID, openTextureID, label, NULL);
}
...
bool ImGui::TreeNodeBehavior_IconText(ImGuiID id, ImGuiTreeNodeFlags flags, ImTextureID textureID, const char* label, const char* label_end)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (display_frame)
    {
        ...
        if (g.LogEnabled)
            LogSetNextTextDecoration("###", "###");
        window->DrawList->AddImage(textureID, text_pos, {text_pos.x + g.FontSize, text_pos.y + g.FontSize});
...
bool ImGui::TreeNodeBehavior_IconText(ImGuiID id, ImGuiTreeNodeFlags flags, ImTextureID closeTextureID, ImTextureID openTextureID, const char* label, const char* label_end)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
